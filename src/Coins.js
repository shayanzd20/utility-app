  ...
  getcoin()
  {
      ...
      .then(data=>{
        ...
        if(data.coins.length > 0)
        {
                  ...
                  {
                    if(data.coins[key]["تغییر"] && data.coins[key]["تغییر"].indexOf("low")>0)
                    {
                      status = "down";
                    }
                    if(data.coins[key]["تغییر"] && data.coins[key]["تغییر"].indexOf("high")>0)
                    {
                      status = "up";
                    }
                    this.state.coinPrices.push({key:i.toString(),label:data.coins[key]["عنوان"],status:status,price:data.coins[key]["قیمت"],...data.coins[key]})
                    i++;
                  }
                ...
                <FlatList
                  ...
                  renderItem={({ item }) =>
                        <View style={styles.currencyMotherBlockStyle}>
                          <View style={styles.currencyChildBlockStyle}>
                            <View style={{ flex:1 ,flexDirection: 'row'}}>
                              <View style={{ flex: 1, backgroundColor: 'transparent' , paddingLeft: 5}}>
                                <Text style={[styles.buttonTextItem,{backgroundColor: 'transparent',fontSize: 15, textAlign: 'center'}]}  >{item["قیمت"]}</Text>
                              </View>
                              <View style={{ flex: 1, backgroundColor: 'transparent', justifyContent: 'center', alignItems: 'center'}}>
                          ...
                          <Image source={require('./../assets/images/coin.png')} style={styles.currencyFlagStyle}/>
                        </View>
                      }
                />
